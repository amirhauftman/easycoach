...
export class PlayersService {
    ...
    async getPlayerById(id: string) {
        ...
        if (player && player.matches && player.matches.length > 0) {
            ...
            (player as any).team_label = player.team_id === latestMatch.home_team_id ? latestMatch.home_team : latestMatch.away_team;
        }
    ...
    async getPlayerMatches(playerId: string) {
        ...
        return player.matches.map(match => ({
            ...match,
            player_events: match.events.filter(event => event.player_id === player.id)
        }));
    }

    private async savePlayerFromApi(apiPlayer: any): Promise<Player | null> {
        const playerId = apiPlayer.player_id || apiPlayer.id || String(apiPlayer.id);
        ...
        const playerData = {
            ...
            fname: apiPlayer.fname || apiPlayer.first_name || apiPlayer.name?.split(' ')[0] || '',
            lname: apiPlayer.lname || apiPlayer.last_name || apiPlayer.name?.split(' ').slice(1).join(' ') || '',
            shirt_number: apiPlayer.shirt_number || apiPlayer.number,
            position: apiPlayer.position || apiPlayer.team_position,
            is_starter: apiPlayer.is_starter !== undefined ? apiPlayer.is_starter : true
        };

        let player: Player;
        if (existing) {
            await this.playerRepository.update(existing.id, playerData);
            const updatedPlayer = await this.playerRepository.findOne({ where: { id: existing.id } });
