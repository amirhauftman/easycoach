...
export class MatchesController {
    ...
    async syncMatches(
        ...
        this.logger.log(`POST /matches/sync - Syncing matches for league ${leagueId}, season ${seasonId}`);
        try {
            const result = await this.matchesService.syncMatchesFromApi(leagueId, seasonId);
            return {
                success: true,
    ...
    async updateCompetitionData() {
        this.logger.log('POST /matches/update-competition - Updating existing matches with competition data');
        try {
            const result = await this.matchesService.updateExistingMatchesWithCompetition();
            return {
                success: true,
    ...
    async getMatchesByPlayer(@Param('playerId') playerId: string) {
        this.logger.log(`GET /matches/player/${playerId} - Fetching matches for player`);
